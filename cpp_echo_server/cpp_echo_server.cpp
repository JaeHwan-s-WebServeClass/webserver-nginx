...
int main(void) {
	...
	server_addr.sin_addr.s_addr = htonl(INADDR_ANY); // typedef __uint32_t	in_addr_t;	/* base type for internet address */
...
changelist 인수는 <sys/event.h>에 정의된 대로 이벤트 구조체 배열에 대한 포인터입니다.
changelist에 포함된 모든 변경 사항은 대기열에서 보류 중인 이벤트를 읽기 전에 적용됩니다.  
nchanges 인수는 변경 목록의 크기를 지정합니다.  

eventlist 인수는 이벤트 구조체 배열에 대한 포인터입니다.  
nevents 인수는 이벤트 목록의 크기를 결정합니다.  
nevents가 0이면 select(2)와 달리 시간 제한이 지정되어 있어도 kevent()가 즉시 반환됩니다. 
